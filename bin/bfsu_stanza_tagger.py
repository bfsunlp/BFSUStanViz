...
class OpenFilesThread(threading.Thread):
    ...
    def workproc(self):
        for each_path in self.path_list:
            encoding = rf.detect_encode(each_path)
...
class ProcessingThread(threading.Thread):
    def __init__(self, model_path, lang, input_files, output_path, job, tag_set, nlp):
        threading.Thread.__init__(self)
        self.num_of_file = 0
        self.breakflag = False
        self.model_path = model_path
    ...
    def workproc(self):
        ...
        for each_file in self.input_files:
            ...
            la.process_file(input_path=each_file,
...
class MainFrame(wx.Frame):
    ...
    def __init__(self, parent, id, lang, language):
        self.lang = lang
    ...
    def MenuAbout(self, event):
        ...
        info.Version = "1.0"
        info.Copyright = "(c) 2022-2023 LIU Dingjia, BFSUNLP Team"
        with codecs.open("./Readme.md", "r", "utf-8") as f:
            about_info = f.read()
        info.Description = wordwrap(about_info, 350, wx.ClientDC(self))
        info.WebSite = ("https://github.com/bfsunlp/bfsustanviz", "BFSUStanviz GITHUB")
    ...
    def OnOpenDir(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            dir_path = dlg.GetPath()
            file_paths = rf.load_dir_files(dir_path)
            self.Disable()
            open_files_work = OpenFilesThread(file_paths)
            self.Enable()
            ...
            if file_paths:
                if self.model_list:
                    ...
                    try:
                        ...
                        dlg = wx.MessageDialog(self, 'Please use a correct model!',
                                               'Warning',
                                               wx.OK | wx.ICON_INFORMATION
                                               # wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_INFORMATION
                                               )
                    ...
                    if nlp:
                        ...
                        self.process_gauge.SetRange(num_of_files)
